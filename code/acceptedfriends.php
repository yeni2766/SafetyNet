...
session_start();
	...
	// Select query to fetch the details of the user that logs in.
...
<div class = "wrapper">
	...
	<div id = "links">
		...
		<a href="blockedaccount.php">Blocked Account</a> 
...
<!--main content of the dashboard and child details -->
		...
		<div class = "container">
			<div class ="row">
				<div class ="col-md-10 shadow-sm p-3 bg-light bb" style = "height:400px;" id = "chartscreen">
					<h1 class="display-4">Accepted Friends</h1>
						<table class="table">
							<thead class="thead">
								<tr>
...
<?php
	...
	// create the database connection
	include('safety.php');
	// Select query to the data to add to table
	...
	$sql1 = "select * from users where email = '$sentfrom' or pin = '$sentfrom'";
	...
	<td>" .$result ['pin']."</td>
	<td><button type='button' class='btn btn-primary' id = 'removebutton'>Remove Friend</button></td>
	<td><button type='button' class='btn btn-primary' id = 'blockbutton'>Block Friend</button></td>
	</tr>
	";
}
}
else {
echo "Your child currently has no friends, head over to the 'Add Friends' page to get your child chatting straight away!";
}
?>
...
$.post(`blockcommand.php?sendcommand=sendRequest&pin=${pin1}`,function(response){
...
//it passes the pin through to a new page that's waiting for a specific action and it will create a notification to the other user
$.post(`removecommand.php?sendcommand=sendRequest&pin=${pin1}`,function(response){
	if(response == 'request sent'){
$('#removebutton').hide();
